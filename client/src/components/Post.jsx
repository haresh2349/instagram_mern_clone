...
const Post = ({ post }) => {
  ...
  const { myProfile } = useSelector((store) => store.AppReducer);
  ...
  const handleDisLike = (postId) => {
    setLike(false);
    dispatch(disLikeThePost(postId));
  };
  ...
  return (
    ...
    >
      <Flex justifyContent={"space-between"} p="10px" alignItems={"center"}>
        ...
        >
          <HStack>
            ...
            />
            <Box>
              <Text fontWeight={"600"}>{post?.postedBy?.username}</Text>
            </Box>
          </HStack>
        </Link>
        <Box>
      ...
      <HStack justifyContent={"space-between"} p="5px 10px">
        <HStack>
          {like === false &&
          post.likes.includes(myProfile?.user?._id) === false ? (
      ...
      <Flex flexDir={"column"} gridGap="10px">
        {post?.comments?.map((comment) => {
          return (
            <HStack key={comment._id} p="5px 10px">
              <Text fontWeight={"600"}>{comment?.postedBy?.username}</Text>
